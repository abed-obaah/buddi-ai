...
export default function PastQuestionsCard() {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        ...
        if (data.status === 200 && data.subjects) {
          const subjectsArray = Object.values(data.subjects).map(
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.status === 200) {
        if (Array.isArray(data.data)) setQuestions(data.data);
        else if (data.data) setQuestions([data.data]);
        else setQuestions([]);
        return data;
      } else {
        setQuestions([]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (questionsData && (questionsData.data || Array.isArray(questionsData.data))) {
        ...
        const questionsArray = Array.isArray(questionsData.data)
          ? questionsData.data
          : [questionsData.data];
          ...
          <body>
            ...
            ${questionsArray.map((q, i) => `
              <div class="question">
                ...
                ${q.section ? `<div class="section">${q.section}</div>` : ''}
                <div class="question-text">${q.question}</div>
                <div class="options">
                  ${Object.entries(q.option || {}).map(([key, val]) =>
                    val ? `<div class="option"><strong>${key.toUpperCase()}. </strong>${val}</div>` : ''
                  ).join('')}
                </div>
                <div class="answer">
                  <strong>Answer: ${q.answer?.toUpperCase() || 'N/A'}</strong>
                  ${q.solution ? `<br><strong>Solution:</strong> ${q.solution}` : ''}
                </div>
              </div>
            `).join('')}
          </body>
        ...
        setTimeout(() => {
          setLoading(false);
          setShowModal(false);
          alert(`Download complete! ${examType} past questions for ${course} (${year})${school ? " - " + school : ""}`);
        }, 500);
      } else {
        clearInterval(progressInterval);
        setLoading(false);
  ...
  const handleOverlayClick = (e) => { if (e.target.id === "modal-overlay") setShowModal(false); };
  const handleCourseSelect = (subject) => { setCourse(subject); setShowCourseDropdown(false); };
  const handleYearSelect = (selectedYear) => { setYear(selectedYear); setShowYearDropdown(false); };
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-gray-700 text-center text-base font-medium">Generate Past Questions</p>
      </div>
      ...
      {showModal && (
        ...
        >
          <div className="relative bg-white rounded-2xl shadow-lg w-[90%] max-w-lg p-6">
            ...
            <h2 className="text-2xl font-semibold mb-4 text-center">Generate Past Questions</h2>

            {loading ? (
              <div className="flex flex-col items-center justify-center py-8">
                <p className="text-gray-700 mb-3">{progress < 90 ? "Fetching questions..." : "Generating PDF..."}</p>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div className="bg-gradient-to-r from-[#CD2574] to-[#E46708] h-3 rounded-full transition-all" style={{ width: `${progress}%` }}></div>
                </div>
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleDownload(); }} className="space-y-4">
                ...
                <div className="relative" ref={courseDropdownRef}>
                  <label className="block text-gray-700 text-sm font-medium mb-2">Course / Subject</label>
                  <button
                    type="button"
                  ...
                  {showCourseDropdown && (
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {subjectsLoading ? (
                        <div className="p-3 text-center text-gray-500">Loading subjects...</div>
                      ) : subjects.map((subject, index) => (
                        <div key={index} onClick={() => handleCourseSelect(subject)} className="flex items-center gap-2 p-3 hover:bg-gray-100 cursor-pointer">
                          <svg viewBox="0 0 6 6" aria-hidden="true" className="size-1.5 fill-green-500 flex-shrink-0"><circle r={3} cx={3} cy={3} /></svg>
                          <span>{subject}</span>
                        </div>
                      ))}
                    </div>
                ...
                {course && (
                  <div className="relative" ref={yearDropdownRef}>
                    ...
                    >
                      <span>{yearsLoading ? "Loading years..." : year || "Select year"}</span>
                      <ChevronDown className="w-4 h-4 text-gray-500" />
                    </button>
                    {showYearDropdown && (
                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                        {yearsLoading ? (
                          <div className="p-3 text-center text-gray-500">Loading available years...</div>
                        ) : years.length === 0 ? (
                          <div className="p-3 text-center text-gray-500">No years available for {course}</div>
                        ) : years.map((yearItem, index) => (
                          <div key={index} onClick={() => handleYearSelect(yearItem)} className="flex items-center gap-2 p-3 hover:bg-gray-100 cursor-pointer">
                            <svg viewBox="0 0 6 6" aria-hidden="true" className="size-1.5 fill-blue-500 flex-shrink-0"><circle r={3} cx={3} cy={3} /></svg>
                            <span>{yearItem}</span>
                          </div>
                        ))}
                      </div>
                    ...
                    {years.length > 0 && !yearsLoading && (
                      <p className="text-xs text-gray-500 mt-1">{years.length} year(s) available for {course}</p>
                    )}
                  </div>
                )}
